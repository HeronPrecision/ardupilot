...
AP_Baro_ICP201XX::AP_Baro_ICP201XX(AP_Baro &baro, AP_HAL::Device &_dev)
    ...
    , dev(&_dev)
{
    // Initialize SPI buffers like Betaflight
...
bool AP_Baro_ICP201XX::read_reg(uint8_t reg, uint8_t *buf, uint8_t len)
{
    ...
    // Set up the SPI transaction exactly like Betaflight
    spi_tx_buf[0] = ICP201XX_SPI_CMD_READ;  // 0x3C - Read command
    spi_tx_buf[1] = reg;                    // Register address
    memset(&spi_tx_buf[2], 0xFF, len);      // Dummy bytes to clock in data
    
    // Perform the SPI transfer
    bool ret = dev->transfer(spi_tx_buf, len + 2, spi_rx_buf, len + 2);
    ...
    if (ret) {
        ...
        if (reg == REG_DEVICE_ID && spi_rx_buf[0] != 0x00) {
            memcpy(buf, &spi_rx_buf[0], len);
            hal.console->printf("ICP201XX: CHIP ID in RX[0]=0x%02X\n", buf[0]);
        } else {
            memcpy(buf, &spi_rx_buf[2], len);
...
bool AP_Baro_ICP201XX::write_reg(uint8_t reg, uint8_t val)
{
    ...
    // Set up the SPI transaction exactly like Betaflight
    spi_tx_buf[0] = ICP201XX_SPI_CMD_WRITE;  // 0x33 - Write command
    spi_tx_buf[1] = reg;                     // Register address
    spi_tx_buf[2] = val;                     // Data value
    
    // Perform the SPI transfer
    bool ret = dev->transfer(spi_tx_buf, 3, spi_rx_buf, 3);
