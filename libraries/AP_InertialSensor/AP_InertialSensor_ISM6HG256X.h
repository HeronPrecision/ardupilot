...
#ifndef ISM6HG256X_DEFAULT_ROTATION
#define ISM6HG256X_DEFAULT_ROTATION ROTATION_NONE
#endif

class AP_InertialSensor_ISM6HG256X : public AP_InertialSensor_Backend {
public:
    enum class ISM6HG256X_Type : uint8_t {
        ISM6HG256X,
    };

    static AP_InertialSensor_Backend *probe(AP_InertialSensor &imu,
                                            AP_HAL::OwnPtr<AP_HAL::SPIDevice> dev,
                                            enum Rotation rotation=ISM6HG256X_DEFAULT_ROTATION);

    ~AP_InertialSensor_ISM6HG256X() override;
...
private:
    enum class FifoTag : uint8_t {
        ...
        Gyro2xC = 0x0C,
        Gyro3xC = 0x0D,
        AccelHG = 0x1D,   // high-g (up to +/-256g) accelerometer output
        Unsupported = 0xFF,
    };
    ...
    AP_InertialSensor_ISM6HG256X(AP_InertialSensor &imu,
    ...
    bool configure_accel_hg(uint16_t rate_hz);
    ...
    Vector3f fuse_lowg_highg(const Vector3f &lowg_raw) const;
    ...
    uint8_t hg_odr_code_for_rate(uint16_t rate_hz) const;
    ...
    float _accel_scale;
    float _gyro_scale;
    // high-g (+/-256g) accelerometer: scale and the most-recent sample (sensor
    // frame, m/s^2). Used to transparently replace low-g axes that have clipped.
    float _accel_hg_scale;
    Vector3f _accel_hg;
    bool _have_hg;    // set once a high-g FIFO word has been seen
    uint8_t _whoami;
    uint32_t _temperature_last_ms;
    uint16_t _backend_rate_hz;
    uint32_t _backend_period_us;
    bool _fast_sampling = false;
    ISM6HG256X_Type _ism6hg256x_type;
